...
function checkLoggingStatus()
{
    ...
    if [[ -z "$(compgen -A function | grep -Ew "(^writeLogEntry)")" ]] && [[ -v "${LOGGING_LOADED}" ]] && [[ "${LOGGING_LOADED}" == "${TRUE}" ]] && [[ -n "${CONFIG_MAP["${logging_type}"]}" ]] && [[ "${CONFIG_MAP["${logging_type}"]}" == "${_TRUE}" ]]; then return 1; fi
}
